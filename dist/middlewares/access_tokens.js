...
const verifyAccessToken = (req, res, next) => {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return next(
      (0, http_errors_1.default)(401, "Access denied. No token provided.")
    );
  }
  ...
  try {
    ...
    return next(
      (0, http_errors_1.default)(403, "Access denied. Invalid token")
    );
  }
};
...
const isAuthenticated = (req, res, next) => {
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    ...
    try {
      ...
      req.userId = decoded.id;
      return next(
        (0, http_errors_1.default)(400, "You are already logged in.")
      );
    } catch (error) {
      next();
    }
